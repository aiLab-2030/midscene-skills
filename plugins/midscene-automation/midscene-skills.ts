...
import { prepareChromeProfile, getPluginRoot } from './src/browser-utils.js';
  ...
  const q = query({
    ...
    options: {
      systemPrompt: {
        type: 'preset',
        preset: 'claude_code',
        append: `\n\n# Browser Automation via Midscene CLI

For browser automation tasks, use bash commands to call the Midscene CLI tool:

**Available commands:**
- \`node dist/src/cli.js navigate <url>\` - Navigate to a URL and take screenshot
- \`node dist/src/cli.js act "<action>"\` - Perform natural language action (e.g., "click the login button")
- \`node dist/src/cli.js query "<query>"\` - Query information from the page (e.g., "what is the page title?")
- \`node dist/src/cli.js assert "<condition>"\` - Assert a condition on the page (e.g., "the login button is visible")
...
- Use \`act\` for interactions (clicks, typing, etc.)
- Use \`query\` to extract information from the page
...
- Intelligent element identification without selectors
- Visual and semantic understanding of web pages
- Support for complex interactions and data extraction
...
async function closeBrowserOnExit() {
  try {
    ...
    const closeProcess = spawn('node', ['dist/src/cli.js', 'close'], {
      stdio: 'inherit'
    });
