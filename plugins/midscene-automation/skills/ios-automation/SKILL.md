...
Automate iOS devices and simulators using natural language through `npx @midscene/cli ios <command>`. This skill leverages Midscene's AI-powered visual understanding to interact with any iOS app without requiring accessibility IDs or XPath selectors.
...
```bash
npx @midscene/cli ios act "<action description>"
```

**Examples:**
```bash
npx @midscene/cli ios act "Tap the Settings icon"
npx @midscene/cli ios act "Swipe up on the screen"
npx @midscene/cli ios act "Type 'hello world' in the search field"
npx @midscene/cli ios act "Scroll down to find the Privacy section"
```
...
```bash
npx @midscene/cli ios query "<query description>"
```

**Examples:**
```bash
npx @midscene/cli ios query "What is the current battery percentage?"
npx @midscene/cli ios query "What are all the visible app names on the home screen?"
npx @midscene/cli ios query "What is the title of the current page?"
```
...
```bash
npx @midscene/cli ios assert "<condition>"
```

**Examples:**
```bash
npx @midscene/cli ios assert "The Settings app is open"
npx @midscene/cli ios assert "The Wi-Fi toggle is turned on"
npx @midscene/cli ios assert "A search bar is visible at the top of the screen"
```
...
```bash
npx @midscene/cli ios screenshot
```
...
```bash
npx @midscene/cli ios screenshot
npx @midscene/cli ios act "Tap the first item in the list"
```
...
```bash
npx @midscene/cli ios act "Tap the Login button"
# Wait briefly for navigation
sleep 2
npx @midscene/cli ios assert "The home screen is displayed"
```
...
```bash
npx @midscene/cli ios act "Toggle the Wi-Fi switch"
npx @midscene/cli ios assert "Wi-Fi is turned off"
```
...
```bash
npx @midscene/cli ios act "Press the Home button"
...
```bash
npx @midscene/cli ios act "Tap the 'General' option in the Settings list"
npx @midscene/cli ios act "Tap the back button in the top left"
```

### Fill a Form
```bash
npx @midscene/cli ios act "Tap the Username text field"
npx @midscene/cli ios act "Type 'john@example.com'"
npx @midscene/cli ios act "Tap the Password text field"
npx @midscene/cli ios act "Type 'securepassword123'"
npx @midscene/cli ios act "Tap the Sign In button"
```

### Verify Screen Content
```bash
npx @midscene/cli ios query "What is the current page title?"
npx @midscene/cli ios assert "The profile page shows the username 'john'"
```
...
- Run `npx @midscene/cli ios connect` to diagnose connectivity.
